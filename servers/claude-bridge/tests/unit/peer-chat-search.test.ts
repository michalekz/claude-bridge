...
describe("peer_chat_search", () => {
	...
	test("scope='all-projects' works without any env gate (v0.4.1+)", async () => {
		...
		const a = uuid();
		await writeSession(projectADir, a, [
			userEvent(a, "2026-05-25T10:00:00Z", "anything"),
		]);
	...
	test("maxMatches caps result + reports truncation", async () => {
		...
		for (let i = 0; i < 20; i++) {
			events.push(
				userEvent(
					s,
					`2026-05-25T10:${i.toString().padStart(2, "0")}:00Z`,
					`MATCH ${i}`,
				),
			);
		}
		...
		beforeEach(async () => {
			...
			await writeSession(projectADir, selfId, [
				userEvent(
					...
					"we agreed on the SENTINEL approach",
				),
			]);
		});
		...
		test("THE REGRESSION: sessions never opened are declared, not counted as misses", async () => {
			...
			for (let i = 0; i < 3; i++) {
				...
				await writeSession(projectADir, id, [
					userEvent(
						id,
						`2026-08-04T12:0${i}:00.000Z`,
						"another SENTINEL mention",
					),
				]);
			}
	...
	describe("the over-cap advice depends on the scope in use", () => {
		...
		test("on scope='project' it does NOT suggest scope='project'", async () => {
			...
			const res = await callSearch(ctx, {
				query: "anything",
				scope: "project",
			});
