...
describe("createPeerRegistry — startHeartbeat / listActivePeers", () => {
	...
	test("flush updates lastSeen without waiting for interval", async () => {
		...
		expect(Date.parse(refreshed.lastSeen)).toBeGreaterThan(
			Date.parse(initial.lastSeen),
		);
		await handle.stop();
	});
	...
	test("update(patch) changes cached name/source for next write", async () => {
		...
		const handle = await reg.startHeartbeat({
			id,
			name: "boot-name",
			pid: 1,
			source: "cwd-slug",
		...
		const content = JSON.parse(
			await readFile(join(baseDir, "status", `${id}.json`), "utf-8"),
		);
	...
	test("listActivePeers returns peers within ONLINE_THRESHOLD_MS (sorted by name)", async () => {
		const reg = createPeerRegistry({ baseDir, intervalMs: 60_000, now });
		const a = await reg.startHeartbeat({
			id: uuid("alice"),
			name: "alice",
			pid: 1,
		});
		const b = await reg.startHeartbeat({
			id: uuid("bob"),
			name: "bob",
			pid: 2,
		});
		...
		expect(list.every((p) => typeof p.id === "string" && p.id.length > 0)).toBe(
			true,
		);
	...
	test("heartbeat filename = <id>.json (peerId, not name)", async () => {
		...
		expect((await stat(join(baseDir, "status", `${id}.json`))).isFile()).toBe(
			true,
		);
