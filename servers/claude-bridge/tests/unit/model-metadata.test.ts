...
describe("normalizeModelId", () => {
	test("strips date suffix", () => {
		...
		expect(normalizeModelId("claude-opus-4-1-20250805")).toBe(
			"claude-opus-4-1",
		);
	});
	...
	test("strips both", () => {
		...
		expect(normalizeModelId("claude-haiku-4-5-[1m]-20251001")).toBe(
			"claude-haiku-4-5-",
		);
	});
...
describe("the table agrees with GET /v1/models", () => {
	test("every model the API serves is in the table", () => {
		const missing = API_SHAPES.map((s) => normalizeModelId(s.id)).filter(
			(id) => !lookupModel(id),
		);
	...
	test("context window and max output match the API for every shared model", () => {
		...
		for (const shape of API_SHAPES) {
			...
			if (m.contextWindow !== shape.max_input_tokens) {
				drift.push(
					`${m.id} context ${m.contextWindow} != api ${shape.max_input_tokens}`,
				);
			}
			if (m.maxOutputTokens !== shape.max_tokens) {
				drift.push(
					`${m.id} maxOutput ${m.maxOutputTokens} != api ${shape.max_tokens}`,
				);
			}
		}
	...
	test("THE REGRESSION: Sonnet 4.5 is a 1M model", () => {
		...
		expect(lookupModel("claude-sonnet-4-5-20250929")?.contextWindow).toBe(
			1_000_000,
		);
	});
