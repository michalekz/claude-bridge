...
describe("emitTerminalTitle", () => {
	...
	test("silently fails on non-existent path (no throw)", () => {
		...
		expect(() =>
			emitTerminalTitle("/no/such/path/fake-tty", "X"),
		).not.toThrow();
	});
...
describe("isTerminalTitleEnabled", () => {
	...
	test("defaults to enabled when env var is empty string", () => {
		expect(
			isTerminalTitleEnabled({ CLAUDE_BRIDGE_EMIT_TERMINAL_TITLE: "" }),
		).toBe(true);
	});

	test('disabled when env var is "0"', () => {
		expect(
			isTerminalTitleEnabled({ CLAUDE_BRIDGE_EMIT_TERMINAL_TITLE: "0" }),
		).toBe(false);
	});

	test('disabled when env var is "false" (case-insensitive)', () => {
		expect(
			isTerminalTitleEnabled({ CLAUDE_BRIDGE_EMIT_TERMINAL_TITLE: "false" }),
		).toBe(false);
		expect(
			isTerminalTitleEnabled({ CLAUDE_BRIDGE_EMIT_TERMINAL_TITLE: "FALSE" }),
		).toBe(false);
	...
	test('enabled for "1" / "true" / arbitrary non-disable values', () => {
		expect(
			isTerminalTitleEnabled({ CLAUDE_BRIDGE_EMIT_TERMINAL_TITLE: "1" }),
		).toBe(true);
		expect(
			isTerminalTitleEnabled({ CLAUDE_BRIDGE_EMIT_TERMINAL_TITLE: "true" }),
		).toBe(true);
		expect(
			isTerminalTitleEnabled({ CLAUDE_BRIDGE_EMIT_TERMINAL_TITLE: "yes" }),
		).toBe(true);
	});
});
