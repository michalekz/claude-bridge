...
describe("encodeProjectDir", () => {
	describe("Linux/macOS", () => {
		test("converts /opt/my-project → -opt-my-project", () => {
			expect(encodeProjectDir("/opt/my-project", "linux")).toBe(
				"-opt-my-project",
			);
		});
		...
		test("handles macOS path", () => {
			expect(encodeProjectDir("/Users/me/projects/foo", "darwin")).toBe(
	...
	describe("Windows", () => {
		test("converts C:\\Users\\me\\proj → C--Users-me-proj", () => {
			expect(encodeProjectDir("C:\\Users\\me\\proj", "win32")).toBe(
				"C--Users-me-proj",
			);
		});
		...
		test("collapses consecutive backslashes", () => {
			expect(encodeProjectDir("C:\\\\Users\\\\me", "win32")).toBe(
				"C--Users-me",
			);
		});

		test("Windows path with spaces (each space → dash, no collapsing)", () => {
			expect(encodeProjectDir("C:\\My Project\\sub dir", "win32")).toBe(
				"C--My-Project-sub-dir",
			);
		});

		test("Windows path with dots (each dot → dash)", () => {
			expect(encodeProjectDir("C:\\Users\\me\\s.r.o", "win32")).toBe(
				"C--Users-me-s-r-o",
			);
		});
		...
		test("Windows path with non-ASCII chars (Czech diacritics → dashes)", () => {
			expect(
				encodeProjectDir(
					"o:\\MICRONIC Přerov s.r.o\\Micronic - Dokumenty\\Marketing",
					"win32",
				),
		...
		test("Windows: literal dashes in path are preserved", () => {
			expect(encodeProjectDir("C:\\foo-bar\\baz", "win32")).toBe(
				"C--foo-bar-baz",
			);
		});
	});
	...
	describe("Linux/macOS with non-alphanumeric chars", () => {
		test("Linux path with spaces (each space → dash)", () => {
			expect(encodeProjectDir("/opt/my project/foo bar", "linux")).toBe(
				"-opt-my-project-foo-bar",
			);
		});
...
describe("path resolvers (current platform)", () => {
	...
	test("sessionFile composes correctly", () => {
		...
		expect(result.endsWith(join("-opt-my-project", "abc-123.jsonl"))).toBe(
			true,
		);
	});
	...
	test("inboxDir composes for peer name", () => {
		expect(
			inboxDir("mantis").endsWith(join(".claude-bridge", "inbox", "mantis")),
		).toBe(true);
	});

	test("peerRegistryFile composes JSON", () => {
		expect(
			peerRegistryFile("mantis").endsWith(
				join(".claude-bridge", "peers", "mantis.json"),
			),
		).toBe(true);
	});
	...
	test("sessionIndexFile points to SQLite", () => {
		expect(
			sessionIndexFile().endsWith(
				join(".claude-bridge", "index", "sessions.sqlite"),
			),
		).toBe(true);
	});
});
