...
describe("normalizeFromOAuth", () => {
	test("returns hasLiveData=true and correct source", () => {
		const s = normalizeFromOAuth(
			makeOAuthEnvelope("2026-07-04T00:07:18Z"),
			FIXED_NOW,
		);
...
describe("normalizeFromStatusLine", () => {
	test("returns hasLiveData=true with source=statusline-stdin", () => {
		const s = normalizeFromStatusLine(
			makeStatusLineEnvelope("2026-07-05T15:30:00Z"),
			FIXED_NOW,
		);
		expect(s.hasLiveData).toBe(true);
				...
				rate_limits: {
					...
					seven_day: {
						...
						resets_at: Math.floor(
							Date.parse("2026-07-06T02:59:59.590Z") / 1000,
						),
					},
				},
			},
