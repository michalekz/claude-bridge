...
function assistantEndTurnEvent(
	sessionId: string,
	ts: string,
	text: string,
): JsonlEvent {
...
function assistantToolUseEvent(
	...
	return {
		...
		message: {
			...
			model: "claude-opus-4-7",
			content: [
				{ type: "tool_use", id: `tu-${counter}`, name: toolName, input: {} },
			],
...
async function writeHeartbeat(
	...
	await writeFile(
		path,
		JSON.stringify({ id: sessionId, lastSeen: new Date().toISOString() }),
	);
	if (ageSeconds > 0) {
...
describe("list_sessions enhancement", () => {
	...
	test("default returns metadata + active flag (no event counts)", async () => {
		...
		await writeSession(sid, [
			userPromptEvent(sid, "2026-01-01T10:00:00Z", "hi"),
		]);
	...
	test("project filter restricts to one project dir", async () => {
		...
		await writeSession(sidA, [
			userPromptEvent(sidA, "2026-01-01T10:00:00Z", "in A"),
		]);
		await writeFile(
	...
	test("limit caps returned session count", async () => {
		for (let i = 0; i < 5; i++) {
			const sid = uuid();
			await writeSession(sid, [
				userPromptEvent(sid, "2026-01-01T10:00:00Z", `s${i}`),
			]);
		}
