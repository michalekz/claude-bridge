...
function withWeekResetsAt(
	base: RateLimitStatus,
	resetsAt: string,
): RateLimitStatus {
...
describe("a borrow across accounts is refused", () => {
	it("the chimera that was observed in production does not form", () => {
		const out = composeFromStatusLineAndOAuth(
			freshAfterRotation(),
			frozenPreviousAccount(),
		);
	...
	it("the refusal is stated, not silent", () => {
		...
		expect(out.secondaryRejected?.freshWeekResetsAt).toBe(
			"2026-08-14T21:00:00.000Z",
		);
		expect(out.secondaryRejected?.olderWeekResetsAt).toBe(
...
describe("the same account still composes", () => {
	it("sub-second skew between the two halves is not a disagreement", () => {
		...
		const fresh = withWeekResetsAt(
			freshAfterRotation(),
			"2026-08-10T03:00:00.000Z",
		);
		const older = withWeekResetsAt(
			frozenPreviousAccount(),
			"2026-08-10T02:59:59.604Z",
		);
	...
	it("a missing or unparseable window is not treated as evidence of anything", () => {
		...
		expect(composeFromStatusLineAndOAuth(fresh, nonsense).source).toBe(
			"composed",
		);
	});
