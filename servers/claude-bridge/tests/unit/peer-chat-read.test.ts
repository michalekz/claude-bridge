...
describe("peer_chat_read", () => {
	...
	test("default returns last 10 messages, text only, drops thinking + tool_use", async () => {
		...
		for (let i = 0; i < 15; i++) {
			...
			if (i % 2 === 0)
				events.push(userEvent(peer.self.id, ts, `user msg ${i}`));
			else
				events.push(
					assistantEvent(peer.self.id, ts, [
						{ type: "text", text: `assistant msg ${i}` },
					]),
				);
		}
	...
	test("lastN respects custom value", async () => {
		...
		for (let i = 0; i < 5; i++) {
			events.push(
				userEvent(peer.self.id, `2026-01-01T10:0${i}:00Z`, `msg ${i}`),
			);
		}
		await writeSession(peer.self.id, events);
	...
	test("rolesOnly:['user'] returns prompt-only view", async () => {
		...
		const events: JsonlEvent[] = [
			...
			assistantEvent(peer.self.id, "2026-01-01T10:01:00Z", [
	...
	test("maxBytes truncates oldest first", async () => {
		...
		for (let i = 0; i < 10; i++) {
			events.push(
				userEvent(peer.self.id, `2026-01-01T10:0${i}:00Z`, `${big}_${i}`),
			);
		}
		await writeSession(peer.self.id, events);
	...
	test("crossProject:true reads any session by UUID", async () => {
		...
		await writeSession(deadId, [
			userEvent(deadId, "2026-01-01T10:00:00Z", "ghost text"),
		]);
	...
	test("reading own session works (post-autocompact recovery use case)", async () => {
		...
		const me = await makeContext("me");
		await registerPeer(me);
		await writeSession(me.self.id, [
			userEvent(me.self.id, "2026-01-01T10:00:00Z", "my own msg"),
		]);

		const byName = await callRead(me, { to: "me" });
		expect(byName.isError).toBeFalsy();
		const { payload: payloadByName } = parseResult(byName);
		const messagesByName = payloadByName["messages"] as Array<
		...
		const messagesById = payloadById["messages"] as Array<
			Record<string, unknown>
		>;
	...
	test("invalid_timestamp on unparseable sinceTimestamp", async () => {
		...
		await writeSession(peer.self.id, [
			userEvent(peer.self.id, "2026-01-01T10:00:00Z", "x"),
		]);

		const result = await callRead(me, {
			to: "peer",
			sinceTimestamp: "not-a-date",
		});
		expect(result.isError).toBe(true);
	...
	test("compact truncates long text at ~180 chars", async () => {
		...
		await writeSession(peer.self.id, [
			userEvent(peer.self.id, "2026-01-01T10:00:00Z", longText),
		]);
	...
	test("json format produces compact JSON (no pretty-print)", async () => {
		...
		await registerPeer(peer);
		await writeSession(peer.self.id, [
			userEvent(peer.self.id, "2026-01-01T10:00:00Z", "hi"),
		]);
	...
	describe("event counts agree with the file on disk", () => {
		test("THE REGRESSION: totalEvents counts lines the schema cannot read", async () => {
			...
			await writeSession(peer.self.id, [
				...
				assistantEvent(peer.self.id, "2026-08-04T10:00:01Z", [
					{ type: "text", text: "hi" },
				]),
				...
				{ type: "mode", sessionId: peer.self.id } as unknown as JsonlEvent,
				{
					...
					sessionId: peer.self.id,
				} as unknown as JsonlEvent,
			]);

			const { ok, payload } = parseResult(
				await callRead(me, { to: "peer-counts" }),
			);
			expect(ok).toBe(true);
			const scanned = payload["scanned"] as Record<
				string,
		...
		test("reading own session is allowed — the description now says so too", async () => {
			...
			await registerPeer(me);
			await writeSession(me.self.id, [
				userEvent(me.self.id, "2026-08-04T10:00:00Z", "own words"),
			]);

			// Legitimate after an autocompact: the transcript holds what context lost.
			const { ok, payload } = parseResult(
				await callRead(me, { to: "me-self-read" }),
			);
			expect(ok).toBe(true);
