...
describe("readLatestTitleFromJsonl — incremental scan", () => {
	...
	const noise = (n: number) =>
		Array.from({ length: n }, (_, i) =>
			line({ type: "user", message: `filler ${i}` }),
		).join("");
	...
	it("picks up a custom-title APPENDED after the first scan — the /rename case", async () => {
		await writeFile(
			file,
			noise(20) + line({ type: "ai-title", aiTitle: "Auto Title" }),
		);
		...
		await appendFile(
			file,
			noise(20) + line({ type: "custom-title", customTitle: "renamed-peer" }),
		);
	...
	it("rescans from scratch when the file SHRINKS below the cached offset", async () => {
		await writeFile(
			file,
			noise(30) + line({ type: "ai-title", aiTitle: "Before Rotation" }),
		);
		...
		await writeFile(
			file,
			line({ type: "ai-title", aiTitle: "After Rotation" }),
		);
	...
	it("takes the LAST title when several are appended over time", async () => {
		...
		await appendFile(
			file,
			line({ type: "custom-title", customTitle: "three" }),
		);
	...
	it("survives a multi-byte character landing exactly on the 256 KB chunk seam", async () => {
		...
		expect(Buffer.byteLength(padLine.slice(0, PREFIX.length + fillerLen))).toBe(
			CHUNK - 1,
		);

		const target = line({
			type: "custom-title",
			customTitle: "žluťoučký-kůň-úpěl",
		});
