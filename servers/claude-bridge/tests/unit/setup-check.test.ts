...
describe("setup-check version comparison", () => {
	...
	test("pre-release ordering by suffix", () => {
		expect(compareVersions("0.9.0-alpha.2", "0.9.0-alpha.1")).toBeGreaterThan(
			0,
		);
...
describe("setup-check statusLine detection", () => {
	test("recognizes wrapper.sh", () => {
		expect(
			isStatusLineConfigured("~/.claude/claude-bridge-statusline-wrapper.sh"),
		).toBe(true);
	});

	test("recognizes direct statusline.cjs invocation", () => {
		expect(
			isStatusLineConfigured("node ~/.claude/claude-bridge-statusline.cjs"),
		).toBe(true);
	});
	...
	test("recognizes CLAUDE_PLUGIN_ROOT path in a hook context (defensive)", () => {
		...
		expect(
			isStatusLineConfigured(
				"node ${CLAUDE_PLUGIN_ROOT}/dist/claude-bridge-statusline.cjs",
			),
		).toBe(true);
	});

	test("does NOT recognize benabraham's status line", () => {
		expect(isStatusLineConfigured("~/.claude/claude-code-status-line.py")).toBe(
			false,
		);
	});
