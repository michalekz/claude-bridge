...
beforeAll(async () => {
  ...
  await pExecFile(
    "npx",
    ["esbuild", SOURCE, "--bundle", "--platform=node", "--target=node18", "--format=cjs", `--outfile=${bundlePath}`],
    { cwd: PKG_ROOT },
  );
}, 60_000);
...
describe("refresh-limits backoff", () => {
  it("selhání curlu se dotkne throttle markeru a nezapíše envelope", { timeout: 20_000 }, async () => {
  ...
  it("čerstvý marker drží throttle i po předchozím selhání (žádný re-touch)", { timeout: 20_000 }, async () => {
    const home = await makeHome();
  ...
  it("úspěch dál zapisuje envelope i marker (regresní stráž fixu)", { timeout: 20_000 }, async () => {
    const home = await makeHome();
