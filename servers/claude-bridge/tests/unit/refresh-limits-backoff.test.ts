...
async function makeHome(): Promise<string> {
	...
	await writeFile(
		...
		JSON.stringify({
			claudeAiOauth: { accessToken: "sk-ant-test-token-not-real" },
		}),
	);
	return home;
}
...
function runHook(home: string, pathDir: string) {
	...
	return pExecFile(
		"/bin/sh",
		["-c", `echo '{}' | ${process.execPath} ${bundlePath}`],
		{
...
const markerPath = (home: string) =>
	join(home, ".claude-bridge", "live", "last-oauth-refresh");
const oauthPath = (home: string) =>
	join(home, ".claude-bridge", "live", "oauth-api.json");
