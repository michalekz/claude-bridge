...
export async function readSessionJsonAt(
	path: string,
...
export async function readLatestTitleFromJsonl(
	jsonlPath: string,
): Promise<string | null> {
	let s: Stats;
	try {
...
export async function resumedSessionIdFromParent(
	...
	try {
		const raw = await readFile(
			join(procRoot, String(ppid), "cmdline"),
			"utf-8",
		);
...
export async function identityFromProc(
	...
	const id =
		resumedOverride ?? (await resumedSessionIdFromParent(ppid, procRoot));
	if (!id) return null;
...
async function awaitSessionJson(
	...
	while (Date.now() < deadline) {
		...
		await new Promise((r) =>
			setTimeout(r, Math.min(250, Math.max(0, deadline - Date.now()))),
		);
	}
	return null;
}
...
export async function resolvePeerIdentity(
	...
	const resumedId =
		opts.resumedSessionId ??
		(await resumedSessionIdFromParent(ppid, opts.procRoot));
