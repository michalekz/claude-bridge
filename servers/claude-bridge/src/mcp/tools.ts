...
function ok(data: unknown): ToolResult {
	return {
		content: [
			{ type: "text", text: JSON.stringify({ ok: true, ...(data as object) }) },
		],
	};
}
...
export async function listProjectsTool(): Promise<ToolResult> {
	try {
		...
		log.error("list_projects_failed", {
...
export async function listSessionsTool(
	...
	try {
		...
		log.error("list_sessions_failed", {
			err: e instanceof Error ? e.message : String(e),
		});
		return err(
...
export async function sessionStatsTool(
	...
	try {
		...
		const filtered = args.project
			? matches.filter((m) => m.projectDir === args.project)
			: matches;
		...
		log.error("session_stats_failed", {
			err: e instanceof Error ? e.message : String(e),
		});
		return err(
...
> {
	...
	if (byShort.length > 1) {
		...
		const own = ownTeam
			? byShort.filter((p) => teamOfName(p.name) === ownTeam)
			: [];
 ...
 */
function peerDiagShape(p: ActivePeer): {
	id: string;
	name: string;
	displayName?: string;
} {
...
function sessionKindOf(pid: number | undefined): string | undefined {
	...
	try {
		const raw = readFileSync(
			join(homedir(), ".claude", "sessions", `${pid}.json`),
			"utf-8",
		);
	...
	try {
		...
		return ok({
			self: {
				 ...
				 */
				...((selfRow?.source ?? ctx.self.source) === "cwd-slug"
					? { nameIsFallback: true }
					: {}),
			},
			count: peers.length,
...
export async function peerInboxReadTool(
	ctx: ServerContext,
): Promise<ToolResult> {
	try {
		...
		return {
			...result,
			content: [...result.content, { type: "text" as const, text: block }],
		};
	} catch (e) {
		log.error("peer_inbox_read_failed", {
			err: e instanceof Error ? e.message : String(e),
		});
		return err(
...
function formatMarkdown(meta: ChatReadMeta, messages: ChatMessage[]): string {
	...
	if (meta.query) {
		...
		const ctx =
			meta.query.contextLines > 0 ? `, ±${meta.query.contextLines} ctx` : "";
		lines.push(
...
async function fileMatchesPrefilter(
	...
	try {
		...
		for (;;) {
			...
			carry =
				text.length > PREFILTER_OVERLAP_CHARS
					? text.slice(-PREFILTER_OVERLAP_CHARS)
					: text;
		}
...
function formatInboxBlock(
	messages: MessageEnvelope[],
	echoed: Set<string> = new Set(),
): string {
	...
	for (const m of messages) {
		...
		const ext = m.from.startsWith("external:")
			? " [external — peer_reply nelze]"
			: "";
		lines.push(
...
async function buildContextStatusEntry(
		...
		sessions.length > 0
			...
			: // No session file yet, but statusLine capture might still exist —
				...
				await readContextUsageForSession([
					{ sessionId: peerId, filePath: "" } as never,
				]);
	...
	if (!usage) {
		...
		return {
			...
			...(placeholder.setupPointer
				? { setupPointer: placeholder.setupPointer }
				: {}),
			...(guard ? { guard } : {}),
		};
	}
	...
	return {
		...
		...(usage.contextLimitCaveat
			? { contextLimitCaveat: usage.contextLimitCaveat }
			: {}),
...
export async function peerContextStatusTool(
	...
	try {
		...
		log.error("peer_context_status_failed", {
			err: e instanceof Error ? e.message : String(e),
		});
		return err(
...
export async function readContextGuard(
	peerId: string,
): Promise<ContextGuardConfig | undefined> {
	try {
...
async function writeContextGuard(
	peerId: string,
...
export async function peerSetContextGuardTool(
	...
	try {
		const current =
			(await readContextGuard(ctx.self.id)) ?? DEFAULT_GUARD_CONFIG;
		...
		log.error("peer_set_context_guard_failed", {
			err: e instanceof Error ? e.message : String(e),
		});
		return err(
...
export async function peerSetNotificationTool(
	...
	try {
		const current =
			(await readNotificationConfig(ctx.self.id)) ??
		...
		log.error("peer_set_notification_failed", {
			err: e instanceof Error ? e.message : String(e),
		});
		return err(
...
export async function rateLimitStatusTool(): Promise<ToolResult> {
	try {
		...
		log.error("rate_limit_status_failed", {
			err: e instanceof Error ? e.message : String(e),
		});
		return err(
...
export async function readRateLimitGuard(): Promise<
	RateLimitGuardConfig | undefined
> {
	try {
...
export async function peerSetRateLimitGuardTool(
	...
	try {
		const current =
			(await readRateLimitGuard()) ?? DEFAULT_RATE_LIMIT_GUARD_CONFIG;
		const next: RateLimitGuardConfig = {
			enabled: args.enabled ?? current.enabled,
			sessionWarnAtPercent:
				args.sessionWarnAtPercent ?? current.sessionWarnAtPercent,
			sessionCriticalAtPercent:
				args.sessionCriticalAtPercent ?? current.sessionCriticalAtPercent,
			weekWarnAtPercent: args.weekWarnAtPercent ?? current.weekWarnAtPercent,
			weekCriticalAtPercent:
				args.weekCriticalAtPercent ?? current.weekCriticalAtPercent,
			notifyPeerIds: args.notifyPeerIds ?? current.notifyPeerIds,
		};
...
export async function modelInfoTool(
	args: z.infer<typeof ModelInfoArgs>,
): Promise<ToolResult> {
	try {
		if (args.model) {
...
export const TOOLS: ToolSpec[] = [
	...
	},
	{
		...
		inputSchema: {
			...
			properties: {
				project: {
					type: "string",
					description:
						"Optional: restrict to a specific project dir (e.g. '-opt-oxy-kb')",
				},
				limit: {
					type: "number",
		...
		handler: async (args) => {
			...
			if (!parsed.success)
				return err("invalid_args", "Schema validation failed", parsed.error);
		...
		inputSchema: {
			...
			properties: {
				inReplyTo: {
					type: "string",
					description:
						"msg-id of the original ask (from piggyback or peer_inbox_read)",
				},
		...
		inputSchema: {
			...
			properties: {
				to: {
					type: "string",
					description:
						"Peer id (UUID, preferred) or display name. Self not allowed.",
				},
				lastN: {
					type: "number",
				...
				sinceTimestamp: {
					type: "string",
					description:
						"ISO 8601 timestamp — only messages at or after this time",
				},
				sinceLastUserPrompt: {
					type: "boolean",
					description:
				...
				maxBytes: {
					type: "number",
					description:
						"Hard cap on output bytes (default 30000). Oldest dropped first.",
					minimum: 1,
					maximum: 1_000_000,
				},
				includeToolCalls: {
				...
				includeThinking: {
					type: "boolean",
					description:
						"Include assistant `thinking` blocks. Default false — often very large.",
				},
				rolesOnly: {
					type: "array",
					items: { type: "string", enum: ["user", "assistant"] },
					description:
						"Restrict to only these roles. E.g. ['user'] for prompt-only view.",
				},
				crossProject: {
					type: "boolean",
					description:
				...
				queryRegex: {
					type: "boolean",
					description:
						"Treat `query` as a regex pattern (case-insensitive). Default false.",
				},
				contextLines: {
					type: "number",
					description:
		...
		inputSchema: {
			...
			properties: {
				...
				queryRegex: {
					type: "boolean",
					description:
						"Treat `query` as a regex pattern. Default false (substring match).",
				},
				scope: {
					type: "string",
				...
				contextLines: {
					type: "number",
					description:
						"Include ±N neighbor messages per match (default 1, max 10).",
					minimum: 0,
					maximum: 10,
				},
				maxMatches: {
				...
				maxBytes: {
					type: "number",
					description:
						"Hard cap on output bytes (default 30000). Sessions truncated last-first.",
					minimum: 1,
					maximum: 1_000_000,
				},
				includeSelf: {
		...
		inputSchema: {
			...
			properties: {
				peer: {
					type: "string",
					description:
						"Session id, full name, or short name in the caller's team",
				},
				team: {
					type: "string",
				...
				set: {
					...
					properties: {
						label: {
							type: "string",
							description:
								"Short display name — tmux window title and projections",
						},
						windowIndex: {
							type: "number",
							description:
								"Requested window position. Recorded only in v0.11.0.",
						},
						model: {
							type: ["string", "null"],
						...
						anthropicBaseUrl: {
							type: ["string", "null"],
							description:
								"How this peer reaches Anthropic. String = route there; null = DELIBERATELY DIRECT; " +
								"omit = nobody decided, fall back to the fleet default. `unset` withdraws the " +
								"declaration and is NOT the same as setting null.",
						},
					},
					additionalProperties: false,
				},
		...
		inputSchema: {
			...
			properties: {
				...
				wait: {
					type: "boolean",
					description:
						"Keep waiting for the verdict. Default false — one look.",
				},
				timeoutMs: {
					type: "number",
		...
		inputSchema: {
			...
			properties: {
				peer: {
					type: "string",
					description:
						"Peer sessionId (UUID) or display name. Prefer sessionId for uniqueness.",
				},
				reason: {
					type: "string",
					description:
						"Optional free-text reason recorded in events.jsonl for the audit trail.",
				},
				force: {
					type: "boolean",
					description:
		...
		inputSchema: {
			...
			properties: {
				...
				displayName: {
					type: "string",
					description:
						"Human name for the tmux session (used as the sessionKey).",
				},
				cwd: {
					type: "string",
				...
				resume: {
					type: "boolean",
					description:
						"Append `--resume <sessionId>` to args. Fork-guard applies.",
				},
				model: {
					type: "string",
				...
				wait: {
					type: "boolean",
					description:
						"Poll for result envelope before returning. Default false.",
				},
				timeoutMs: {
					type: "number",
					minimum: 1,
		...
		inputSchema: {
			...
			properties: {
				...
				model: {
					type: "string",
					description:
						"Override the model on the new instance (default: carry over from state).",
				},
				accountProfile: {
					type: "string",
					description:
						"Override account profile (default: carry over from state).",
				},
				wait: { type: "boolean" },
		...
		inputSchema: {
			...
			properties: {
				...
				verbose: {
					type: "boolean",
					description:
						"Include full per-peer fields (default false → compact view).",
				},
				wait: {
					type: "boolean",
	...
	},
	{
		...
		description:
			...
			"🔴 **A peer cannot compact itself.** `/compact` is a built-in CLI command, not a tool — no peer can invoke it from inside its own turn. Only a human at the keyboard or the daemon through `tmux send-keys` can. A peer asked to compact itself can flush its anchor and say it is ready; the inject has to come from outside.\n\n" +
		...
		inputSchema: {
			...
			properties: {
				...
				anchorTimeoutMs: {
					...
					maximum: 300000,
					description:
						"Wait budget for the ack file (default 300000 — three of these numbers " +
						"disagreed until v0.11.23; the daemon has always used 300000). A peer " +
						"writing a real anchor takes minutes, not seconds.",
				},
				ackPollMs: {
					type: "number",
					minimum: 1,
				...
				idleTimeoutMs: {
					...
					maximum: 600000,
					description:
						"How long to wait for the peer to go idle AFTER it acked (default 90000). " +
						"Writing the ack is itself a turn, so before v0.11.33 the single probe " +
						"0.4 s later saw `busy` almost every time and refused. Pass 0 for that " +
				...
				idlePollMs: {
					...
					maximum: 10000,
					description:
						"Gap between idle probes (default 1000). `claude agents --json` costs " +
						"~600 ms, so a shorter gap only queues probes.",
				},
				skipAnchorRequest: {
					type: "boolean",
					description:
				...
				verifyTimeoutMs: {
					...
					description:
						...
						"(default 360000). It said 180000 here until 2026-09-01, five days after the " +
						"daemon moved to 360 s — the number a caller reads was HALF the number the " +
						"daemon uses, and the caller who trusts it plans around a budget that is not " +
						"the one in force. Measured on 24 compacts of one session: 6 ran longer than " +
						"180 s, the slowest 220 s. And the duration does NOT track the token count — " +
						"706k took 206 s while 929k took 128 s — so this is not a number to tune to " +
						"the largest peer. It is a generous ceiling, and an expired one is a " +
						"statement about the watch, not about the compact.",
				},
				reason: {
					type: "string",
					description: "Free-text reason recorded in events.jsonl.",
		...
		inputSchema: {
			...
			properties: {
				...
				prune: {
					type: "boolean",
					description:
						"Stop peers not in the spec. Default false = safe (keep extras).",
				},
				inline: {
					type: "object",
					description:
		...
		inputSchema: {
			...
			properties: {
				...
				dryRun: {
					type: "boolean",
					description:
						"Preview the stop order and parameters without touching any peer.",
				},
				inline: {
					type: "object",
					description:
		...
		inputSchema: {
			...
			properties: {
				team: {
					type: "string",
					description:
						"Team the adopted peers are recorded under. Required — adoption stamps it.",
				},
				mode: {
					type: "string",
					enum: ["auto", "manual"],
		...
		inputSchema: {
			...
			properties: {
				peers: {
					...
					items: { type: "string" },
					description:
						"Peer ids or names to release. Mutually exclusive with `team`.",
				},
				team: {
					type: "string",
					description:
						"Release every peer under this team. Mutually exclusive with `peers`.",
				},
		...
		inputSchema: {
			...
			properties: {
				team: {
					type: "string",
					description:
						"Restrict the record check to one team. Unmanaged peers are still listed.",
				},
				markDead: {
					type: "boolean",
					description:
		...
		inputSchema: {
			...
			properties: {
				peers: {
					...
					items: { type: "string" },
					description:
						"Peer ids or names, in restart order. Mutually exclusive with `team`.",
				},
				team: {
					type: "string",
				...
				dryRun: {
					type: "boolean",
					description:
						"DEFAULT TRUE. Returns the order and launch parameters, restarts nothing.",
				},
				wait: { type: "boolean", description: "Default true." },
