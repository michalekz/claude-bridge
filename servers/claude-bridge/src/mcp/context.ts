...
export async function buildContext(
	...
	const self =
		opts.identity ??
		(await resolvePeerIdentityWithRetry({
			...(opts.identityOptions ?? {}),
			skipTitleScan: true,
		}));
	log.info("identity_resolved", {
		id: self.id,
		name: self.name,
		source: self.source,
	});
...
async function migrateIdentity(
	...
	try {
		...
		if (code !== "ENOENT") {
			log.warn("inbox_dir_migrate_failed", {
				err: e instanceof Error ? e.message : String(e),
			});
		}
 ...
 */
export async function pumpInboxToChannel(
	ctx: ServerContext,
